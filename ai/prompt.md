...
<!-- file: spec/env/profiles/base.yml -->
...
<!-- file: spec/env/profiles/dev.yml -->
...
<!-- file: spec/env/profiles/prod.yml -->
...
RULES
- When code references local files (relative imports), create missing files with minimal but working implementation AND tests.
- Do not modify files under 'spec/'.
- Place tests under 'tests/'. Put source under language-idiomatic directories (e.g. 'src/').
- Produce runnable, cohesive changes.

### Missing local dependencies (detected by scanner)




 (or .rb)
 (map to .py or /__init__.py)
