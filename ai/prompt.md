...
- Read the specs above and *generate implementation code* and/or *tests*.
- You MUST produce at least one file change.
- Default output location is 'generated/'. Create new files there unless the spec explicitly maps to existing paths.
- Prefer TypeScript/Node.js 20 if language is not specified; otherwise follow the spec.
- If information is missing, generate a best-effort scaffold with TODO comments.

## OUTPUT CONTRACT (STRICT)

- Return a single Unified Diff patch rooted at the repository.
- No prose, no explanations, no code fences, no headings.
- Begin with 'diff --git' or with '--- a/' then '+++ b/'.
- DO NOT return a no-op. If unsure, synthesize a minimal scaffold under 'generated/'.
